...
parser.add_argument('--nodbcommit', help='whether the json commit to the database should be skipped',
                    action='store_true')
parser.add_argument('--dbhost', help='a database host address/name to connect to', required=False, default='localhost')
# parser argument for yaml - to pass on more sample metadata into the database
parser.add_argument("-y", "--sample-yaml", dest="sample_config", type=str, required=False,
...
outfolder = os.path.abspath(os.path.join(args.output_parent, fileid))

# gzip the original bed file and keep it with the pipeline results
try:
    dst_path = os.path.abspath(os.path.join(outfolder, bedfile_name))
    ...
    if not os.path.exists(link_dest):
        os.symlink(bedfile_name + ".gz", link_dest)
...
if not args.nodbcommit and os.path.splitext(bedfile_name)[1] != '':
    ...
    try:
        ...
        with open(json_file_path, 'r', encoding='utf-8') as f:
            ...
            pm.info("Inserting metadata into database: {}".format(args.dbhost))
