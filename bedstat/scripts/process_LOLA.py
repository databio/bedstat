        ...
        try:
            ...
            if len(n) == 1:
                ...
                if l[0].find(",") != -1:
                    ...
                    raise Exception(
                        "Not tsv and not csv format for %s. Aborting." % idx_file
                    )
            else:
                # tsv case
            ...
            for row in reader:
                try:
                    ...
                    for kw in idx_file_keywords:
                        if kw not in row:
                            ...
                            if kw == "filename":
                                ...
                                s = os.path.abspath(
                                    os.path.join(bed_path_base, row["filename"])
                                )
                                f = Path(s)
                    ...
                    fn_err = os.path.abspath(
                        os.path.join(bed_path_base, row["filename"])
                    )
                    msg = "Skipping line #{} in index file {}, file {} not found.\n".format(
                        reader.line_num, idx_file, fn_err
                    )
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Script to process LOLA database into PEP format."
    )

    parser.add_argument(
        "--lola_loc",
        help="full path to base LOLA db (folder whose subfolders are hg38, hg19 etc.",
    )
